...
from l2cs import L2CS, select_device, Gaze360, Mpiigaze, Mpiigaze2
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='datasets/MPIIFaceGaze4/Image', type=str)
    parser.add_argument(
        '--gazeMpiilabel_dir', dest='gazeMpiilabel_dir', help='Directory path for gaze labels.',
        default='datasets/MPIIFaceGaze4/Label', type=str)
    ...
    parser.add_argument(
        ...
        default='output/snapshots/temp', type=str)
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if data_set=="gaze360":
        ...
        for fold in range(15):
            model, pre_url = getArch_weights(args.arch, 90)
            state_dict = torch.load("L2CSNet_gaze360.pkl", map_location='cuda')  # hoặc 'cuda'
            model.load_state_dict(state_dict)

            model.to(gpu)
            print('Loading data.')
            dataset=Mpiigaze2(testlabelpathombined,args.gazeMpiimage_dir, transformations, True, fold, args.gaze360label_dir, args.gaze360image_dir)
            train_loader_gaze = DataLoader(
                dataset=dataset,
                batch_size=int(batch_size),
                shuffle=True,
            ...
            for epoch in range(num_epochs):
                ...
                if epoch % 1 == 0 and epoch < num_epochs:
                    save_dir = output + '/fold' + str(fold)
                    os.makedirs(save_dir, exist_ok=True)  # Đảm bảo thư mục tồn tại
                    save_path = os.path.join(save_dir, '_epoch_' + str(epoch + 1) + '.pkl')
